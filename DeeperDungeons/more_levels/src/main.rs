...
impl State {
    ...
    fn advance_level(&mut self) {
        //START: get_player_entity
        let player_entity = *<Entity>::query()
            .filter(component::<Player>())
            .iter(&mut self.ecs)
            .nth(0)
            .unwrap();
        //END: get_player_entity
        ...
        //END: store_carried
        //START: obliterate
        let mut cb = CommandBuffer::new(&mut self.ecs);// <callout id="co.deeper.command_buffer" />
        for e in Entity::query().iter(&self.ecs) {// <callout id="co.deeper.entity_query" />
            if !entities_to_keep.contains(e) {// <callout id="co.deeper.contains" />
                cb.remove(*e);
            }
        }
        cb.flush(&mut self.ecs);// <callout id="co.deeper.cbflush" />
        //END: obliterate
